...
def prepare_dataloader(train_df, test_df, batch_size, task, target_name='target', dl_generator=None):
    
    if task == 'classification':
        # when target is one-hot encoded, target_cols is list, otherwise, target_cols is str (taget_name) 
        target_cols = [c for c in train_df.columns.tolist() if target_name in c]
        ...
        train_dataset = torch.utils.data.TensorDataset(torch.tensor(train_df.drop([target_name], axis=1).values, dtype=torch.float32),
                                                        torch.tensor(train_df[target_name].values, dtype=torch.float32))
        test_dataset  = torch.utils.data.TensorDataset(torch.tensor(test_df.drop([target_name], axis=1).values, dtype=torch.float32),
                                                    torch.tensor(test_df[target_name].values, dtype=torch.float32))
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, 
                                                worker_init_fn=seed_worker, generator=dl_generator)
        test_loader  = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False)
        full_train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=len(train_df), shuffle=True, 
...
def mean_absolute_error(input, target):
    ...
    return torch.sum(torch.abs(input - target)) / len(target)
...
def evaluation(loader, model, device, num_classes):
    ...
    Args:
        ...
        device:
        num_classes: int
            1 -> regression, 2 -> binary-classification, 3 -> multi-class classification
    '''
    soft_max = F.softmax
    ...
    if num_classes >=2: # classification
        ...
        all_pred_vals = torch.tensor([], dtype=torch.float32)
        for instances, labels in loader:
            instances, labels = instances.to(device), labels.to(device)
            outputs = model(instances)

            all_labels = torch.cat([all_labels, labels.detach().cpu().to(torch.float32)])
            all_pred_vals = torch.cat([all_pred_vals, outputs.detach().cpu().to(torch.float32).flatten()])

    score_dict = {}
    if num_classes == 1: # regression
        score_dict['mse'] = mF.mean_squared_error(input=all_pred_vals, target=all_labels)
        score_dict['mae'] = mean_absolute_error(input=all_pred_vals, target=all_labels)
...
def train_est_model(
        est_method, lr, bs, ep, wd, device, 
        est_target_name, 
        weak_train_df, weak_test_df, ord_train_df, ord_test_df,
        hidden_dim=None, loss_func=None,
        seed=42,
    ...
    input_dim = len(weak_train_df.columns) - 1
    ...
    if est_method == 'mlp':
        ...
        model.eval()
        with torch.no_grad():
            train_loss = loss_check(loader=weak_train_loader, model=model, device=device, loss_func=loss_func, num_classes=num_classes)
            test_loss =loss_check(loader=loop_ord_test_loader, model=model, device=device, loss_func=loss_func, num_classes=num_classes)

        if verbose:
            print('Epoch: 0. Tr Loss: {}. Te Loss: {}'.format(train_loss, test_loss))
        ...
        for epoch in range(ep):
            ...
            if verbose:
                ...
                print('Epoch: {}. Tr Loss: {}. Te Loss: {}'.format(epoch+1, train_loss, test_loss))
            save_table[epoch, :] = epoch+1, train_loss, test_loss
        
        model.eval()
        with torch.no_grad():
            train_score_dict = evaluation(loader=weak_train_loader, model=model, device=device, num_classes=num_classes)
            test_score_dict = evaluation(loader=ord_test_loader, model=model, device=device, num_classes=num_classes)
            train_score_dict['loss'] = loss_check(loader=weak_train_loader, model=model, device=device, loss_func=loss_func, num_classes=num_classes)
            test_score_dict['loss'] = loss_check(loader=ord_test_loader, model=model, device=device, loss_func=loss_func, num_classes=num_classes)
...
def train_pred_model(arch, lr, bs, ep, wd, device, 
    ...
    input_dim = len(train_df.columns) - 1 # -1 is target
    ...
    if verbose:
        if num_classes >=2: # classification
            ...
            print('Epoch: {}. Tr Loss: {}. Te Loss: {}'.format(0, train_loss, test_loss))
